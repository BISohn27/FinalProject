...
public class FinanceService {
	...
	public Map<String,Object> getSalesByTime(int eno, String periodType, String startPeriod, String endPeriod){
		...
		if(periodType.equals("Year")) {
			...
			for (int i = 8; i < 24; i++) {
				obj[i - 8] = mapper.getSalesByTimeY(eno, i, startPeriod, endPeriod);
				if (obj[i - 8] != null) {
					labelList.add(i + "~" + (i + 1));
					salesList.add(((BigDecimal) obj[i - 8]).intValue());
				}
			}
		} else if(periodType.equals("Custom Period")) {
			Object[] obj = new Object[16];
			...
			for (int i = 8; i < 24; i++) {
				obj[i - 8] = mapper.getSalesByTimeC(eno, i, startPeriod, endPeriod);
				if (obj[i - 8] != null) {
					labelList.add(i + "~" + (i + 1));
					salesList.add(((BigDecimal) obj[i - 8]).intValue());
				}
	...
	public Map<String,Object> getMenuSalesByTime(int eno, String periodType, String startPeriod, String endPeriod, String menu){
		...
		if(periodType.equals("Year")) {
			Object[] obj = new Object[16];
			for (int i = 8; i < 24; i++) {
				obj[i - 8] = mapper.getMenuSalesByTimeY(eno, i, startPeriod, endPeriod, menu);
				if (obj[i - 8] != null) {
					labelList.add(i + "~" + (i + 1));
					salesList.add(((BigDecimal) obj[i - 8]).intValue());
				}
			...
			for (int i = 8; i < 24; i++) {
				obj[i - 8] = mapper.getMenuSalesByTimeC(eno, i, startPeriod, endPeriod, menu);
				if (obj[i - 8] != null) {
					labelList.add(i + "~" + (i + 1));
					salesList.add(((BigDecimal) obj[i - 8]).intValue());
				}
...
//if(periodType.equals(pt[0])) {
//	
//}else if(periodType.equals(pt[1])) {
//	
//}
