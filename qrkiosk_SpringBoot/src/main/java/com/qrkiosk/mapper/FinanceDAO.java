...
public interface FinanceDAO {
	...
	@Select("select sum(count*price) from enterprise_report_view where eno=${eno} and mname=#{menu} and (year(otime) = ${year}) group by mname")
	public Object getMenuSalesByYear(@Param("eno")int eno, @Param("year")int year, @Param("menu")String menu);
	@Select("select sum(count*price) from enterprise_report_view where eno=${eno} and mname=#{menu} and Year(otime) = ${year} and Month(otime) >= ${month1} and Month(otime) <= ${month2} group by mname")
	...
	@Select("select sum(count*price) from enterprise_report_view where eno=${eno} and mname=#{menu} and Year(otime) = ${year} and Month(otime) =${month} group by mname")
	public Object getMenuSalesByMonth(@Param("eno")int eno, @Param("month")int month1,@Param("year")int year, @Param("menu")String menu);
	@Select("select sum(count*price) from enterprise_report_view where eno=${eno} and mname=#{menu} and (date_format(otime, '%H') = ${time}) and (Year(otime) between ${start} and ${end}) group by mname")
